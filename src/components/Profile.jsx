...
function Profile() {
    ...
    const mytweet=()=>{
        ...
        .then(function (response) {
            ...
            let newdata = response.data.filter((item) => item.userId === id);
            console.log(newdata);
            const latest=newdata.sort((a,b)=>b.id - a.id);
    ...
    const deleteTweet = async () => {
        try {
            ...
            settweet(tweet.filter((item) => item.id !== tweetIdToDelete));
            closeModal();
        } catch (error) {
  ...
  return (
    ...
    <div className='border border-[gray] w-full flex flex-col items-end flex-grow'>
        ...
        <div className='w-full h-[20%] px-2 flex flex-row-reverse justify-between items-end max-sm:justify-center max-sm:gap-8'>
            ...
            <button className='btn bg-transparent border-none w-[10%] font-bold' onClick={()=>{navigate('/Like')}}>الإعجابات</button>
        </div>
    </div>
